...
export const useAccountsWizard = (
  ...
  const isPrevious = useMemo<boolean>(() => currentStep > 0, [currentStep]);
  ...
  useEffect(() => {
    ...
    if (
      ...
      !ValidateAccountsWizardFileStep2.validate(state).error
    ) {
      setCompletedStep(1);
      testConnection();
      return;
    ...
    if (
      state.client !== ServerTypeEnum.SQLITE &&
      !ValidateAccountsWizardStep2.validate(state).error
    ) {
      setCompletedStep(1);
      testConnection();
      return;
